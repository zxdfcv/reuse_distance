...
int main() { //同时回顾下命名空间的使用->C++第八讲
    ...
    std::vector<std::unordered_map<long long, int>> hash_map(1024);
...
//        nodes[rank] = Tree::Insert(time, nodes[rank], sector);
    ...
    printf("%.3f", (double) success / (hit.size() - 1));//注意匹配
    return 0;
}
//第二阶段：多个set
//地址%4->4棵树 取模运算
